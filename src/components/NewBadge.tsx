...
// decorative -- no focus, no click. `.badge-shine` (globals.css) adds the
// periodic sweep; `overflow-hidden` clips it to the badge's own bounds.
export default function NewBadge() {
  return (
    <span
      aria-hidden="true"
      className="nes-btn is-warning badge-shine absolute -top-3 -right-3 rotate-6 py-1 px-2 font-pixel text-[9px] text-black select-none pointer-events-none overflow-hidden"
    >
      NEW
    </span>
  );
